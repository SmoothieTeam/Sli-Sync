...
function VideoEditPage({ videoLoader, slideIndexLoader, onSubmit }) {
  ...
  const [currentSlideIndexes, setSlideIndexes] = useState(
    slideIndexLoader.getIndexes(id)
  );
  ...
  return (
    <div>
      ...
      <div className="video_index">
        {currentSlideIndexes.map((slideIndex, i) =>
          SlideIndexEditor({
            slideIndex,
            onSeek: handleSeeking,
            onChange: (t) => handleChangeTime(i, t),
          })
        )}
      </div>
      <Link to={"/view/" + id} type="submit" onClick={handleSubmit}>
        Submit
      </Link>
    </div>
  );
